...
public class SavePaymentRefitHandlerTests
{
    ...
    {
        var refitAPIMock = new Mock<IForexAPIRefit>();
        var handler = new SavePaymentRateRefitHandler(refitAPIMock.Object);

        var command = new Faker<AddPaymentRefitCommand>()
        ...
        refitAPIMock.Verify(x => x.GetRates(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
    ...
    public async void Handle_CallForexAPI_WhenCurrencyAreNotEqual()
    {
        ...
        refitAPIMock.Setup(x => x.GetRates(Currency,TargetCurrency))
            .ReturnsAsync(0.8M);
        ...
        refitAPIMock.Verify(x => x.GetRates(Currency, TargetCurrency), Times.Once);
        result.Reference.Should().Be(command.Reference);
        result.Amount.Should().Be(command.Amount);
        result.Currency.Should().Be(command.Currency);
        result.TargetCurrency.Should().Be(command.TargetCurrency);
